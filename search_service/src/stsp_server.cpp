...
class Map_params
{
    public:
        ...
        Map_params(double maxx, double maxy, double minx, double miny);
            //xyreso(0.5),yawreso(0.2), xmin(-12.0),xmax(12.0),ymin(-15.0),
            //ymax(12.0), sensor_range(10.0)
            //{
                //xw = int(round((xmax - xmin) / xyreso));
                //yw = int(round((ymax - ymin) / xyreso));
            //}
            //:xyreso(0.5),yawreso(0.2), xmin(minx),xmax(maxx),ymin(miny),
...
public:
  ...
  m_params(NULL)
  {
    nh_.param("MAX_X", MAX_X, {16.5});
    nh_.param("MIN_X", MIN_X, {-15.0});
    nh_.param("MAX_Y", MAX_Y, {20.0});
    nh_.param("MIN_Y", MIN_Y, {-35.0});
    ...
    nh_.param("NUM_AGENT", NUMAGENTS, {1});
 ...
 //publishers for agents
     ...
     as_.start();
     ROS_INFO("Single-search action_started");
  }
...
{
  ros::init(argc, argv, "singlesearch_action");
