...
{
    public:
        Map_params():xyreso(0.5),yawreso(0.2), xmin(-11.0),xmax(10.0),ymin(0.5),
...
public:
  ...
  m_params(NULL)
  {
    ...
    agent1_pose_sub=nh_.subscribe<geometry_msgs::PoseStamped>("/global_pose_a1",10,&MultiSearchManager::global_pose_a1_callback,this);
    agent1_localmap_sub= nh_.subscribe<nav_msgs::OccupancyGrid>("/costmap_node/costmap/costmap",10,&MultiSearchManager::agent1_localmap_callback,this);
    agent2_pose_sub=nh_.subscribe<geometry_msgs::PoseStamped>("/global_pose",10,&MultiSearchManager::global_pose_hsr_callback,this);
  ...
  void executeCB(const search_service::MultiSearchGoalConstPtr &goal)
  {
    ...
    if(called_once)
    {
        ...
        agents_history.poses.push_back(agent1_pose.pose);
        agents_history.poses.push_back(agent2_pose.pose);
    }
    ...
    if(call_multiplan_service(agent1_path, agent2_path))
    {
        ...
        int maxidx2 = calc_IG_trjs_with_leadtrj(agent2_path, agent1_path[maxidx], num_points);
        if(maxidx2>0)
    ...
    //agent1_move_cancel_pub.publish(cancel_msg);
    ...
    bool call_multiplan_service_single(std::vector<geometry_msgs::PoseArray>& path_agent1)
    {
     ...
     if(agent1_history.poses.size()>0)
       ...
       if(search_entropy>0.45)
       {
          ...
          int idx=0; 
          goal_sampling_frontiers(agent1_pose, goals_agent1, idx);
