...
class Map_manager(object):
    ...
    def __init__(self, name):
        ...
        # self.transform = self.tf_buffer.lookup_transform(_OFFSET_TF, _ORIGIN_TF,rospy.Time(0),rospy.Duration(1.0))
        ...
        # self.spot_goal_pub = rospy.Publisher('spot/search_goal', OccupancyGrid,queue_size=10)
    ...
    def execute_cb(self, goal):
        ...
        if self.entropy>0.4:
            ...
            self.result_.goal = self.choose_goal_from_trj(best_trj, self.params_searchmap, self.horizon)
        else:
            self.goals = self.goal_sampling_frontiers()
            if len(self.goals)>0:
