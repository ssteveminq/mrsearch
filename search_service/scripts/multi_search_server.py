...
class Map_manager(object):
    ...
    def __init__(self, name):
        ...
        self.load_yamlfile()
        self.agent1_ori=self.start_pose.pose.orientation
        self.agent2_ori=self.start_pose2.pose.orientation
        # self.goal_sampling_waypoints(self.params_searchmap)
        self.agent1_history=[]
        self.agent2_history=[]
        self.entropy=1.0
        self.occ_entropy=0.0
        self.near_goal_agent1=False
        self.near_goal_agent2=False
        ...
        #agent1 : a1
        agent1_pose_topic='global_pose_a1'
        #agent2 : hsr
        agent2_pose_topic='global_pose'
        rospy.Subscriber(agent1_pose_topic, PoseStamped, self.agent1_pose_Cb)
        ...
        # frontiers_pose_topic_agent1='frontier_poses_agent1'
        # frontiers_pose_topic_agent2='frontier_poses_agent2'
        # rospy.Subscriber(frontiers_pose_topic, PoseArray, self.frontier_poses_Cb)
        ...
        self._as.start()
        rospy.loginfo("multi-search action_started")
    ...
    def call_service(self):
        ...
        # print("agent1_pose[0]", self.agent1_pose[0])
        # print("agent1_pose[1]", self.agent1_pose[1])

        if self.called_once==False:
            self.start_pose.pose.position.x=self.agent1_pose[0]+0.15
            self.start_pose.pose.position.y=self.agent1_pose[1]+0.2
        else:
        ...
        if self.called_once==False:
            self.start_pose2.pose.position.x=self.agent2_pose[0]+0.15
            self.start_pose2.pose.position.y=self.agent2_pose[1]+0.2
        else:
            self.start_pose2.pose.position.x=self.last_goal2.pose.position.x+0.1
            self.start_pose2.pose.position.y=self.last_goal2.pose.position.y+0.1

        self.start_pose.pose.orientation.w=1.0
        self.start_pose2.pose.orientation.w=1.0

        print("agent1_startpose", self.start_pose)
        print("agent2_startpose", self.start_pose2)

        if self.entropy<0.4:
            self.goals = self.goal_sampling_waypoints(self.params_searchmap, self.agent1_pose)
            self.goals2 = self.goal_sampling_waypoints(self.params_searchmap, self.agent2_pose)
        else:
            self.goals = self.goal_sampling_frontiers(self.params_searchmap, self.agent1_pose)
            self.goals2 = self.goal_sampling_frontiers(self.params_searchmap, self.agent2_pose)
        # print(self.goals)
        # print(self.goals2)
        #k:w
    ...
    def goal_sampling_frontiers(self,params_searchmap, robot_pose):
        self.goals=[]
        ...
        for k in range(num_angle):
            temppos=[0,0]
            temppos[0]= robot_pose[0]+ math.cos(k*ang_reso)*(dist+random.uniform(-0.5,0.5))+random.uniform(-0.5,0.5)
            temppos[1]= robot_pose[1]+math.sin(k*ang_reso)*(dist+random.uniform(-0.5,0.5))+random.uniform(-0.5,0.5)
    ...
    def goal_sampling_waypoints(self, params_searchmap, robot_pose):
        dist_threshold = 2.0
    ...
    def execute_cb(self, goal):
        ...
        if self.called_once==False:
                self.agent1_history.append([self.agent1_pose[0], self.agent1_pose[1]])
                self.agent2_history.append([self.agent2_pose[0], self.agent2_pose[1]])

        #if self.no_count<5:
        #check 
        if self.near_goal_agent1==False and self.called_once==True:
            rospy.loginfo("Robot is not close to the previous goal. wait until it's near to that goal")
            rospy.sleep(2.5)
        ...
        # waypoints approach only used when entropy is greater than 0.3
        if self.entropy>0.25:
            ...
            if len(res.multiplan.poses)>0:
                ...
                sp_gtrjs = self.trjs_to_sample(gtrjs,self.agent1_history, self.horizon)
                ...
                while len(trjs_candidate)<1:
                    # self.agent1_history=[]
                    horizon+=5
                    sp_gtrjs = self.trjs_to_sample(gtrjs,self.agent1_history, 15)
                    for gtrj in sp_gtrjs:
                        trjs_candidate.append(gtrj)
                    if horizon>50:
                        self.agent1_history=[]
            else:
                rospy.loginfo("make multimultiplan failed")
                # local_trjs = lane_state_sampling_test(self.agent1_pose, self.params_searchmap, self.search_region)
            ...
            if len(res.multiplan2.poses)>0:
                gtrjs = self.Convert_Poses2trjs(res.multiplan2.poses)
                sp_gtrjs = self.trjs_to_sample(gtrjs,self.agent2_history, self.horizon)
                for gtrj in sp_gtrjs:
                ...
                while len(trjs_candidate)<1:
                    # self.agent1_history=[]
                    horizon+=5
                    sp_gtrjs2 = self.trjs_to_sample(gtrjs,self.agent2_history, 15)
                ...
                rospy.loginfo("make multimultiplan failed")
                # local_trjs2= lane_state_sampling_test(self.agent2_pose, self.params_searchmap, self.search_region)
            ...
            self.publish_selected_path(best_trj2)
            self.result_.goal = self.choose_goal_from_trj(best_trj, self.params_searchmap, self.agent1_history,self.horizon)
            self.result_.goal2 = self.choose_goal_from_trj(best_trj2, self.params_searchmap, self.agent2_history,self.horizon)
        else:
            print("entropy small case --!")
            # self.goals = self.goal_sampling_frontiers()
            # if len(self.goals)>0:
                # FIXME
                # num=random.randint(0, len(self.goals)-1) #randomly select the goal Fixme
                # self.result_.goal=self.goals[num];
                # while num!=num2:
                    # num2=random.randint(0, len(self.goals)-1) #randomly select the goal Fixme
                # self.result_.goal2=self.goals[num2];
        ...
        self.last_goal2 = self.result_.goal2
    ...
    #agent1_pose call back w.r.t {map}
    def agent1_pose_Cb(self, msg):
        #HSR pose call back
        # print("execute_cb")
        # rospy.loginfo("a1_pose_callback")
        self.agent1_pose[0]=msg.pose.position.x
        self.agent1_pose[1]=msg.pose.position.y
        ...
        self.agent1_pose[2]=yaw

        self.start_pose.pose.position.x=self.agent1_pose[0]
        self.start_pose.pose.position.y=self.agent1_pose[1]
        self.publish_search_map()
        if self.called_once:
            x_err = self.last_goal.pose.position.x-self.agent1_pose[0]
            y_err = self.last_goal.pose.position.y-self.agent1_pose[1]
            temp_dist=0.0;
            temp_dist+=math.pow(x_err,2)
            temp_dist+=math.pow(y_err,2)
            temp_dist=math.sqrt(temp_dist)
            if temp_dist<1.75:
                self.near_goal_agent1=True
            else:
                self.near_goal_agent1=False


    #agent2_pose call back w.r.t {map}
    def agent2_pose_Cb(self, msg):
           
        # rospy.loginfo("hsr_pose_callback")
        ...
        self.start_pose2.pose.position.x=self.agent2_pose[0]+0.1
        self.start_pose2.pose.position.y=self.agent2_pose[1]+0.15
 
        # self.publish_search_map()

        if self.called_once:
            x_err = self.last_goal2.pose.position.x-self.agent2_pose[0]
            y_err = self.last_goal2.pose.position.y-self.agent2_pose[1]
            temp_dist=0.0;
            temp_dist+=math.pow(x_err,2)
            temp_dist+=math.pow(y_err,2)
            temp_dist=math.sqrt(temp_dist)
            if temp_dist<1.75:
                self.near_goal_agent2=True
            else:
                self.near_goal_agent2=False
            #rospy.loginfo("near goal: ", self.near_goal_agent1)
    ...
    def Convert_Poses2trjs(self, posesarray):
        print("len:", len(posesarray))
        trjs=[]
        for path in posesarray:
            xs=[]
            ys=[]
            for pose_idx in range(len(path.poses)):
                if pose_idx%5==0 or (pose_idx==len(path.poses)-1):
    ...
    def calc_IG_trjs(self, trj_candidates, params_global, horizon=15):
    ...
    def trjs_to_sample(self,trjs,history, horizon=20):
        ...
        for i, sp in enumerate(trjs):
            ...
            for j, i_s in enumerate(s):
                ...
                if horizon>0 and j<(2.0*horizon):
                    rx.append(ix)
                    ry.append(iy)
                if horizon<0:
                    rx.append(ix)
