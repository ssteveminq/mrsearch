...
table_path = dir_path+"/lookuptables.csv"
...
def check_point_obstaclefree(pos_x, pos_y,occ_map):
    #occ_map = ros_occmap
    map_idx = Coord2Idx(pos_x, pos_y, occ_map.info)
    if occ_map.data[map_idx]>0:
        return False
    else:
        return True
...
def lane_state_sampling_test(cur_states, params_global, search_map):
    k0 = 0.0
    l_center = 1.0
    l_center2 = -1.0
    l_center3 = 1.0
    ...
    for table in result:
        ...
        last_x = xc[-1]
        last_y = yc[-1]
        #check with search map for static obstacle
        # for obs in obstacles:
        if check_point_obstaclefree(last_x, last_y, search_map)==False:
            obstacle_free=False
    ...
    for trj in trjs:
        ...
        b = np.array([xtemp_rots, ytemp_rots])
        c = np.array([xtemp_rots, ytemp_rots])
        b = np.reshape(b, (2, int((b.size)/2)))
        c = np.reshape(c, (2, int((c.size)/2)))


        # print("xtemp_rots", xtemp_rots)
        # print("ytemp_rots", ytemp_rots)
        # b = np.hstack((xtemp_rots, ytemp_rots))
        # c = np.hstack((xtemp_rots, ytemp_rots))
    ...
    print("lenth of ltrjs: " , len(totaltrjs))
