...
# sys.path.remove('/usr/lib/python2.7/dist-packages')
# print(sys.path)
# sys.path.remove('/home/mk/anaconda3/envs/py27/lib/python2.7/dist-packages')
# sys.path.append('/opt/ros/melodic/lib/python2.7')
# sys.path.append('/usr/lib/python2.7/dist-packages')
# sys.path.remove('/usr/lib/python2.7/dist-packages')
import random
import math
...
ColorSet=['red', 'green', 'blue','yellow', 'black', 'cyan', 'green', 'blue','yellow', 'black', 'cyan' ]
...
class tsp_manager:
    ...
    # def __init__(self):
        ...
        #genetic parameters
        self.population_size = pop_size_    # population size (even number!)
        self.generations = 2000       # population's self.generations
    ...
    def fitness(self,creature):
        ...
        for j in range(len(creature)):
            ...
            # print("creature[j]", creature[j])
            # print("self.engineers", self.engineers)
            # print("path", path)
            if len(path) != 0:
                ...
                # print("max(sum_dist)", np.max(sum_dist))
    ...
    def crossover_mutation(self, population, birth_prob):
        ...
        # print("osn", round(len(population)/2))

        for i in range(int(round(len(population)/2))):
            rp = np.random.rand(birth_prob.size)
            # print("random-prob", rp)
            # print("birth_prob", birth_prob)
    ...
    def plot_paths(self, paths, color_, ax=None):
        # plt.clf()
        # plt.title('Best path overall')
        if ax!=None:
            # ax.cla()
            ...
            for v in range(self.atms_number):
                ax.scatter(self.points_locations[v+self.service_centers, 0], self.points_locations[v+self.service_centers, 1], c=color_)
            for v in range(len(paths)):
                if len(paths[v]) != 0:
                    ...
                    ax.plot(path_locations[:, 0], path_locations[:, 1], color=color_)
        else:
            # for v in range(self.service_centers):
    ...
    def get_path(self):
        ...
        # print("best_mean_creature_result ", best_mean_creature_result )
        # print("best_creature_result ", best_creature_result )
        best_selection_prob = self.birth_prob(fitness_result)
        # print("best_selection_prob ", best_selection_prob )
        ...
        if best_path!=None:
            #sorting path based on the distances
            best_path=best_path[0]
            sorted_path = sorted(((v,i) for i, v in enumerate(self.dist[0])),reverse=False)
            min_idx=sorted_path[0][1]
            # print("min_idx", min_idx)
            # print("best[idx]", best_path.index(min_idx))
            s_idx = best_path.index(min_idx)
            os_idx = s_idx
            best_path_sort =[0]*len(best_path)
            # print("best_path_sort",best_path_sort )
            best_path_sort[0]=best_path[s_idx]
            iter=1
            while s_idx<len(best_path)-1:
                s_idx=s_idx+1
                best_path_sort[iter]=best_path[s_idx]
                iter=iter+1
            for k in range(os_idx):
                best_path_sort[iter]=best_path[k]
                iter=iter+1

            # print("best_path_sort",best_path_sort)
            return [best_path_sort]
        else:
