...
class Map_manager(object):
    ...
    def call_service(self):
        if len(self.agent_history)>0:
            ...
            self.start_pose.pose.position.x=self.agent2_pose[0]+0.25
            self.start_pose.pose.position.y=self.agent2_pose[1]+0.3
        ...
        # else:
            # self.goals = self.goal_sampling_frontiers()
        #self.goals = self.goal_sampling_frontiers(self.params_searchmap)
                ...
                if search_idx< len(self.search_region.data):
                    if local_map.data[map_idx]!=-1 and self.search_region.data[search_idx]!=l_occ: #localmap
                        if local_map.data[map_idx]==0:
                            self.search_region.data[search_idx]= l_free #update it as a free
                        elif local_map.data[map_idx]>0:
                            self.search_region.data[search_idx]= l_docc
                        # else:
                        # self.search_region.data[search_idx]= l_occ
    ...
    def execute_cb(self, goal):
        ...
        if self.entropy>0.3:
            ...
            self.goals = self.goal_sampling_frontiers()
            if len(self.goals)>0:
                self.result_.goal=self.goals[0];
            else:
    ...
    def agent2_pose_Cb(self, msg):
        ...
        try: 
            ...
            # self.listener.waitForTransform(_ORIGIN_TF,_OFFSET_TF,rospy.now(), rospy.Duration(3.0))
            transform = self.tf_buffer.lookup_transform(_OFFSET_TF, _ORIGIN_TF,rospy.Time(0),rospy.Duration(1.0))
            pose_map=tf2_geometry_msgs.do_transform_pose(msg,transform)
            # print("pose_map", pose_map)

        except:
            rospy.loginfo("transformPosefailed")
            
        self.agent2_pose[0]=pose_map.pose.position.x
        self.agent2_pose[1]=pose_map.pose.position.y
        self.agent2_ori= pose_map.pose.orientation
        robot_orientation=pose_map.pose.orientation
    ...
    def calc_IG_trjs_old(self, trj_candidates, params_global, horizon=20):
        #Calculating Information gain on trajectories
        #1) calculating sampling point for time horizon 
        #2) calculating FOV region over sampling points 
        #3) Collecting IG gain for overlapped region
    ...
    def trjs_to_sample_old(self,trjs,history, horizon=20):
        spline_trjs=[]
        for i, sp in enumerate(trjs):
            ds = 0.3
            s = np.arange(0, sp.s[-1], ds)
    ...
    def choose_goal_from_trj(self, best_trj, params_globalmap, history,horizon=20):
        ...
        if len(best_trj[0])>horizon and horizon>0:
            if not self.near_history(best_trj[0][horizon], best_trj[1][horizon], history):
                ...
                idx=horizon
                while (self.near_history(best_trj[0][horizon], best_trj[0][horizon], history)==False) and (idx<len(best_trj[0])-1):
                    idx=idx+1
