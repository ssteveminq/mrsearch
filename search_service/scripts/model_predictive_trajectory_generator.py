...
def calc_J(target, p, h, k0):
    ...
    d1 = np.matrix((dp - dn) / (2.0 * h[0, 0]), dtype='float').T
    ...
    xn, yn, yawn = motion_model.generate_last_state(
        p[0, 0], p[1, 0] - h[1, 0], p[2, 0], k0)
    dn = calc_diff(target, [xn], [yn], [yawn])
    d2 = np.matrix((dp - dn) / (2.0 * h[1, 0]), dtype='float').T
    ...
    xn, yn, yawn = motion_model.generate_last_state(
        p[0, 0], p[1, 0], p[2, 0] - h[2, 0], k0)
    dn = calc_diff(target, [xn], [yn], [yawn])
    d3 = np.matrix((dp - dn) / (2.0 * h[2, 0]), dtype='float').T
...
def optimize_trajectory(cur_states, target, k0, p):
    ...
    for i in range(max_iter):
        ...
        dc = np.matrix(calc_diff(target, xc, yc, yawc), dtype='float').T
