...
//added by mk
FrontierSearch::FrontierSearch(costmap_2d::Costmap2D &costmap, int min_frontier_size, std::string &travel_point, double potential_scale, double gain_scale) :
    costmap_(costmap), min_frontier_size_(min_frontier_size), travel_point_(travel_point),potential_scale_(potential_scale),gain_scale_(gain_scale) {
        //added by mk
    }
...
std::vector<Frontier> FrontierSearch::searchFrom(geometry_msgs::Point position){
    ...
    //added by mk for cost
    //for(std::vector<Frontier>::iterator iterator = frontier_list.begin(), end = frontier_list.end(); iterator != end; ++iterator) {
        //(*iterator).cost=frontierCost(*iterator);
    //} 
