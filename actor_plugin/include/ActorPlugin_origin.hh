...
//#include "gazebo/physics/Skeleton.hh"
...
namespace gazebo
{
  class GZ_PLUGIN_VISIBLE ActorPlugin : public ModelPlugin
  {
    ...
    private: void Load_trajectory(sdf::ElementPtr _sdf);
    double vMax;
    double maxAngleUpdate;
    int waypoint_length;
    int waypoint_iter;
    std::vector<physics::TrajectoryInfo> ptrajInfo;
    std::vector<ignition::math::Vector3d> waypointset;
    std::map<unsigned int, common::PoseAnimation*> ptrajectories;
    std::map<double, ignition::math::Pose3d> points;
    std::map<double, ignition::math::Pose3d>::iterator point_iter;
