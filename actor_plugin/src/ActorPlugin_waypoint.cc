...
void ActorPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
 ...
 ROS_INFO("speed!! %d", _sdf->HasElement("speed"));
 ROS_INFO("pose!! %d", _sdf->HasElement("pose"));
 ROS_INFO("skin!! %d", _sdf->HasElement("skin"));
 ...
 //ROS_INFO("trajectory!! %d", _sdf->HasElement("trajectory"));
  //ROS_INFO("something wrong");
  //Load_Script(_sdf->GetElement("script"));
  Load_Script(_sdf);
  //read trajectory and waypoint

}

void ActorPlugin::Load_Script(sdf::ElementPtr _sdf)
{
 ...
 //this->actor->active = this->actor->autostart;
    ...
    if (_sdf->HasElement("trajectory"))
  {
    ...
    while (trajSdf)
    {
      //if (this->actor->SkeletonAnimations().find(trajSdf->Get<std::string>("type")) ==
              //this->actor->SkeletonAnimations().end())
      //{
      ...
      //ROS_INFO("trajectory id : %d", tinfo.id);
      //tinfo.type = trajSdf->Get<std::string>("type");
      //ROS_INFO("trajectory type : %s", tinfo.type.c_str());
      //std::vector<TrajectoryInfo>::iterator iter = this->ptrajInfo.begin();
      //while (iter != this->ptrajInfo.end())
      //{
        //if (iter->id > tinfo.id)
          //break;
        //++iter;
      ...
      //unsigned int idx = iter - this->ptrajInfo.begin();
      //this->ptrajInfo.insert(iter, tinfo);
      ...
      if (trajSdf->HasElement("waypoint"))
      {
        ...
        //common::PoseAnimation *anim = new common::PoseAnimation(animName.str(),
                                                          //last->first, false);
        //this->ptrajInfo[idx].duration = last->first;
        //this->ptrajInfo[idx].translated = true;
        ...
        //{
          ...
          //{
            ...
            //key->Rotation(ignition::math::Quaterniond(pIter->second.Rot().Euler()));
          //}
        ...
        //this->ptrajectories[this->ptrajInfo[idx].id] = anim;
      //}
      //else
      //{

        //auto skelAnims = this->actor->SkeletonAnimations();
        //this->ptrajInfo[idx].duration =
               //(skelAnims[this->ptrajInfo[idx].type]->GetLength());
        //this->ptrajInfo[idx].translated = false;
      //}
    ...
    //for(map_iter;map_iter!=points.end();map_iter++)
    //{
        //std::cout<<"time: "<<map_iter->first <<", way_point"<<map_iter->second.Pos().X()
            //<<map_iter->second.Pos().Y()<<map_iter->second.Pos().Z()<<std::endl;
        //ROS_INFO("time %.2f", map_iter->first);
...
void ActorPlugin::Reset()
{
  ...
  if (this->sdf && this->sdf->HasElement("target"))
    this->target = this->sdf->Get<ignition::math::Vector3d>("target"); //should be the first waypoint
  else
