...
void ActorPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  //ROS_INFO("start location: X: %.2lf, Y: %.2lf", this->start_location.X(),this->start_location.Y());
  ...
  else
  {
    ...
    this->trajectoryInfo->duration = 3.0;
...
void ActorPlugin::OnUpdate(const common::UpdateInfo &_info)
{
  ...
  //ROS_INFO(" %s actor pos x: %.2lf , y: %.2lf --OnUpdates",this->actor->GetName().c_str(), pose.Pos().X(), pose.Pos().Y());
  //ROS_INFO("target pos x: %.2lf , y: %.2lf --OnUpdates", this->target.X(), this->target.Y());
  //std::cout<<"actor pos x: "<<pos.X()<<", y: "<<pos.Y()<<std::endl;
  ...
  //ROS_INFO("velocity: %.2lf",this->velocity);
  ...
  //ROS_INFO(" %s actor updated pos x: %.2lf , y: %.2lf --OnUpdates",this->actor->GetName().c_str(), pose.Pos().X(), pose.Pos().Y());
...
bool ActorPlugin::SetPoseCallback(actor_services::SetPose::Request& req,
  ...
  //ROS_INFO("actor pos x: %.2lf , y: %.2lf --SetPoseCallbackfunction ", pos.X(), pos.Y());
  //std::cout<<"actor pos x: "<<pos.X()<<", y: "<<pos.Y()<<std::endl;
  res.x = pos.X();
  res.y = pos.Y();
  if (req.set_flag == true)
