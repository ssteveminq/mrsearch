    ...
    robot_state_()
{
  ...
  viz_pub=nh_.advertise<nav_msgs::OccupancyGrid>("viz_search_map", 10, true);
  ...
  inspect_target_pub=nh_.advertise<geometry_msgs::PoseArray>("inspection_poses", 10, true);
  stfpose_sub=nh_.subscribe<geometry_msgs::PoseArray>("stf_poses",10,&inspection_manager::posearry_callback,this);
  search_map_sub=nh_.subscribe<nav_msgs::OccupancyGrid>("/search_map",10,&inspection_manager::search_map_callback,this);
  agent_map_sub=nh_.subscribe<nav_msgs::OccupancyGrid>("fov_map",10,&inspection_manager::agent1_localmap_callback,this);
  global_pose.resize(3,0.0);

  //camera region
  camera_visible_region.info.width=10;
  camera_visible_region.info.height= 10;
  camera_visible_region.info.resolution=0.5;
  camera_visible_region.info.origin.position.x=-2.5;
  camera_visible_region.info.origin.position.y=-2.5;
...
void inspection_manager::publish_inspection_poses()
{
    ...
    for(int i(0); i<pose_array.poses.size();i++)
    {
        ...
        if( (pose_array.poses[i].position.x > min_map_x) &&
                (pose_array.poses[i].position.x < max_map_x) &&
                (pose_array.poses[i].position.y < max_map_y) &&
                (pose_array.poses[i].position.y > min_map_y) )
        {
          std::cout << "STF pose exists inside search map" << std::endl;
...
void inspection_manager::search_map_callback(const nav_msgs::OccupancyGrid::ConstPtr& msg)
{
    ...
    max_map_x = msg->info.origin.position.x + (msg->info.width*msg->info.resolution)/2.;
    max_map_y = msg->info.origin.position.y + (msg->info.height*msg->info.resolution)/2.;
    min_map_x = msg->info.origin.position.x - (msg->info.width*msg->info.resolution)/2.;
    min_map_y = msg->info.origin.position.y - (msg->info.height*msg->info.resolution)/2.;

    std::cout << "Map bounds (xmin xmax ymin ymax): " << max_map_x << ", " << min_map_x <<
        ", " << max_map_y << ", " << min_map_y << std::endl;
