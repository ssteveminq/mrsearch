   ...
   isTargetDetected(false), Is_Searchmap_received(false), isActionActive(false), costmap_size_x(100),costmap_size_y(100)
{
 ...
 //Initialize human_belief_type_map
  ...
  frontier_posearray_pub = nh_.advertise<geometry_msgs::PoseArray>("/frontier_poses", 10, true);
  ...
  //globalpose_sub=nh_.subscribe<geometry_msgs::PoseWithCovarianceStamped>("spot/amcl_pose",10,&frontier_manager::global_pose_callback,this);
  globalpose_sub=nh_.subscribe<geometry_msgs::PoseStamped>("global_pose_a1_121",10,&frontier_manager::global_pose_callback,this);
  //as_.start();
}
...
//void frontier_manager::global_pose_callback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg)
...
=======
=======
   //map_en_to_map= tf_buffer.lookupTransform("map_en", "map", ros::Time(0), ros::Duration(2.0));
   geometry_msgs::PoseStamped tmp_pose;
   tmp_pose.header.stamp= ros::Time(0);
   tmp_pose.header.frame_id= "map_en";
   tmp_pose.pose.position.x = msg->pose.position.x;
   tmp_pose.pose.position.y = msg->pose.position.y;
   tmp_pose.pose.orientation= msg->pose.orientation;
    ...
    while(!bfs.empty()){
        ...
        BOOST_FOREACH(unsigned nbr, nhood4(idx)){
            ...
            if(costmap_[nbr] <= costmap_[idx] && !visited_flag[nbr]){
                ...
                if(new_frontier.size > max_frontier_size_){
                    std::vector<frontier_exploration::Frontier> sfrontiers;
                    ROS_INFO("Split_frontiers");
                    SplitFrontiers(new_frontier, pos, sfrontiers);
                    for(size_t k(0);k<sfrontiers.size();k++)
                        frontier_list.push_back(sfrontiers[k]);

                    ROS_INFO("Split_frontiers done");
                }
                else if(new_frontier.size > min_frontier_size_){
...
void frontier_manager::SplitFrontiers(const frontier_exploration::Frontier& frontier, unsigned int reference_,  std::vector<frontier_exploration::Frontier>& newfrontiers)
{
    newfrontiers.clear();
...
void frontier_manager::visualizeFrontiers(const std::vector<frontier_exploration::Frontier>& frontiers)
{
     ...
     //for(size_t j(0);j<frontier_pairs.size();j++)
     //{
        //ROS_INFO("Before pair index: distance = idx: %d, dist:  %.3lf", frontier_pairs[j].first, frontier_pairs[j].second);
        //ROS_INFO("frontier position x: %.3lf, y: %.3lf: ", frontiers[j].travel_point.x, frontiers[j].travel_point.y);
     ...
     //for(size_t j(0);j<frontier_pairs.size();j++)
        //ROS_INFO("After pair index: distance = idx: %d, dist:  %.3lf", frontier_pairs[j].first, frontier_pairs[j].second);
     //human_candidates_distances.resize(candidate_size,0.0);
     //for(size_t i(0);i<candidate_size;i++)
     //{
           //human_candidates_distances[i]=getDistance_from_Vec(global_pose,
