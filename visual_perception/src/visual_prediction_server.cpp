#include "visual_prediction_server.h"
...
// constructor
visual_prediction_manager::visual_prediction_manager(std::string name):as_(nh_, name, boost::bind(&visual_prediction_manager::executeCB, this,_1), false), 
   ...
   isTargetDetected(false), Is_Searchmap_received(false), isActionActive(false), costmap_size_x(100),costmap_size_y(100)
{
  ...
  searchmap_sub =nh_.subscribe<nav_msgs::OccupancyGrid>("/search_map", 10, &visual_prediction_manager::searchmap_callback,this);
  globalpose_sub=nh_.subscribe<geometry_msgs::PoseWithCovarianceStamped>(agent1_pose_topic,10,&visual_prediction_manager::global_pose_callback,this);
...
// destructor
visual_prediction_manager::~visual_prediction_manager()
{
    delete []costmap_;
    costmap_ = NULL;
  // delete sequencer
...
void visual_prediction_manager::executeCB(const visual_perception::VisualUnknownSearchGoalConstPtr &goal)
  {
      double ros_rate = 2;
      ros::Rate r(ros_rate);
      isActionActive=true;
...
void visual_prediction_manager::Mapidx2GlobalCoord(int map_idx, std::vector<double>& global_coords)
{
...
void visual_prediction_manager::publish_searchmap()
{
...
void visual_prediction_manager::publish_target_belief()
{
...
void visual_prediction_manager::searchmap_callback(const nav_msgs::OccupancyGrid::ConstPtr& msg)
{
    // Copy Data;
...
bool visual_prediction_manager::check_staticObs(float x_pos,float y_pos)
{
...
int visual_prediction_manager::Coord2CellNum(double _x, double _y, const nav_msgs::OccupancyGrid& inputmap_)
{	
...
double visual_prediction_manager::getDistance_from_Vec(std::vector<double> origin, double _x, double _y)
{
  double temp=0.0;
...
void visual_prediction_manager::global_pose_a1_callback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
...
//
void visual_prediction_manager::global_pose_callback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg)
{
...
void visual_prediction_manager::target_poses_callback(const geometry_msgs::PoseArray::ConstPtr& msg)
{
...
std::vector<unsigned int> visual_prediction_manager::nhood4(unsigned int idx){
...
std::vector<unsigned int> visual_prediction_manager::nhood8(unsigned int idx){
...
void visual_prediction_manager::parseparameters(ros::NodeHandle n)
  {
...
void visual_prediction_manager::LoadPositionfromConfig(ros::NodeHandle n, std::string input_locations)
{
...
bool visual_prediction_manager::nearestCell(unsigned int &result, unsigned int start, unsigned char val){
...
unsigned char* visual_prediction_manager::getCharMap() const
{
    return costmap_;
}

std::vector<int> visual_prediction_manager::FindCellFrom(geometry_msgs::Point position, int value_)
{
...
bool visual_prediction_manager::isNewFrontierCell(unsigned int idx, const std::vector<bool>& frontier_flag){
...
bool visual_prediction_manager::isNewUnknown(unsigned int idx, const std::vector<bool>& frontier_flag){
    ...
    //Unknown cells should not have at least one cell in 4-connected neighbourhood that is not free
    BOOST_FOREACH(unsigned int nbr, nhood4(idx)){
...
bool visual_prediction_manager::isCellValueandFree(unsigned int idx, int value_){
...
bool visual_prediction_manager::hasFreenbr(unsigned int idx){
...
double visual_prediction_manager::frontierCost(const frontier_exploration::Frontier& frontier)
{
  return (potential_scale_ * frontier.min_distance *map_res) -
         (gain_scale_ * frontier.size * map_res);
}

void visual_prediction_manager::SplitFrontiers(const frontier_exploration::Frontier& frontier, unsigned int reference_,  std::vector<frontier_exploration::Frontier>& newfrontiers)
{
    newfrontiers.clear();
...
frontier_exploration::Frontier visual_prediction_manager::buildNewUnknown(unsigned int initial_cell, unsigned int reference_, std::vector<bool>& visited_flag){
...
/*
void visual_prediction_manager::getNextFrontiers(const std::vector<frontier_exploration::Frontier>& frontiers)
{
...
void visual_prediction_manager::visualizeFrontiers(const std::vector<frontier_exploration::Frontier>& frontiers)
{
...
void visual_prediction_manager::human_poses_callback(const geometry_msgs::PoseArray::ConstPtr& msg)
{
...
void visual_prediction_manager::update_human_occ_belief(int update_type){
...
int visual_prediction_manager::CoordinateTransform_Global2_staticMap(float global_x, float global_y)
{
...
int visual_prediction_manager::CoordinateTransform_Global2_beliefMap(double global_x, double global_y)
{
...
bool visual_prediction_manager::Comparetwopoistions(std::vector<double> pos,std::vector<double> pos2, double criterion)
{
...
void visual_prediction_manager::insert_unknownmap(const std::vector<int> idx_set)
{
...
void visual_prediction_manager::find_avg_pose(const std::vector<int> idx_set, geometry_msgs::Pose& avg_point)
{
    double tmp_x=0.0; 
    double tmp_y=0.0; 
...
std::vector<frontier_exploration::Frontier> visual_prediction_manager::Unknown_search(geometry_msgs::Point position, std::vector<bool>& visited_flag, int target_value_=0 )
{
    unsigned int target_mapidx=(unsigned int) CoordinateTransform_Global2_beliefMap(position.x, position.y);
    unsigned int search_size =Target_Search_map.info.width*Target_Search_map.info.height;
...
void visual_prediction_manager::FindUnknowns()
{
    ...
    while((true_ratio<0.975) && (iter_<max_iter))
    {
        ...
        for(auto frontier :frontier_list)  
        {
            ...
            {
                auto yaw = i*2*MATH_PI/NUM_POS;
                tf2::Quaternion tmp_quat;
                tmp_quat.setRPY(0.0,0.0, yaw);
                tmp_quat=tmp_quat.normalize();

                avg_pose.orientation.x=tmp_quat.getX();
                avg_pose.orientation.y=tmp_quat.getY();
                avg_pose.orientation.z=tmp_quat.getZ();
                avg_pose.orientation.w=tmp_quat.getW();
...
double visual_prediction_manager::get_visited_ratio(std::vector<bool>& visited_flag)
{
...
// filter loop
void visual_prediction_manager::spin()
{
  while (ros::ok())
  {
...
int main(int argc, char **argv)
{
  ...
  visual_prediction_manager f_manager("getunknowns");
  f_manager.spin();
