...
#include <visual_perception/UnknownSearchResult.h>
#include <visual_perception/UnknownSearchFeedback.h>
...
public:
  /// constructor
  prediction_manager(std::string name);
  prediction_manager();
  /// destructor
  virtual ~prediction_manager();
  ...
  void executeCB(const visual_perception::UnknownSearchGoalConstPtr &goal);

private:

  ros::NodeHandle nh_;
  std::string action_name_;
  actionlib::SimpleActionServer<visual_perception::UnknownSearchAction> as_;
  visual_perception::UnknownSearchResult result_;
  visual_perception::UnknownSearchFeedback feedback_;
