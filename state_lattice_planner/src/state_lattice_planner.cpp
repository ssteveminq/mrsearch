...
void StateLatticePlanner::generate_biased_polar_states(const int n_s, const Eigen::Vector3d& goal, double target_velocity, std::vector<Eigen::Vector3d>& states )
{
    ...
    for(int i=0;i<n_s;i++){
        ...
        //mk
        _params.length = 2.2;
        if (goal_distance<_params.length)
            _params.length = 1.;
...
void StateLatticePlanner::generate_biased_polar_states_double(const int n_s, const Eigen::Vector3d& goal, double target_velocity, std::vector<Eigen::Vector3d>& states )
{
    /*
     * n_s: param for biased polar sampling
     */
...
bool StateLatticePlanner::generate_trajectories(const std::vector<Eigen::Vector3d>& boundary_states, const double velocity, const double angular_velocity, const double target_velocity, std::vector<MotionModelDiffDrive::Trajectory>& trajectories)
{
    ...
    //std::cout<<"trjaectory_num: "<<trajectory_num<<std::endl;
    ...
    for(int i=0;i<trajectory_num;i++){
        ...
        //std::cout<<"trj num: "<< i<< ", cost:" << cost<<std::endl;;
    ...
    //std::cout<<"trajectories_.size() = "<<trajectories_.size()<<std::endl;
    //if(count>0){
        int t_count=0;
    ...
    std::cout<<"erased trajectories_.size() = "<<t_count<<std::endl;
...
bool StateLatticePlanner::pickup_trajectory(const std::vector<MotionModelDiffDrive::Trajectory>& candidate_trajectories, const Eigen::Vector3d& goal, MotionModelDiffDrive::Trajectory& output)
{
    ...
    //pickup min_diff_yaw trajectories...
...
bool StateLatticePlanner::check_collision(state_lattice_planner::ObstacleMap<int>& map, const std::vector<Eigen::Vector3d>& trajectory)
{
    /*
    ...
    {
        map.get_xy_from_index(j, coord);
        if(coord[0]>0.0 and fabs(coord[1]<2.0))
            std::cout<<"indx" <<j<<", x: "<<coord[0]<<", y: "<<coord[1]<<",map value:  "<< map.data[j] <<std::endl;
    }
    ...
    std::cout<<"bresenhams_line size " << size <<std::endl;
    for(int i=0;i<(size);i++){
    std::cout<<"x: "<< bresenhams_line[i](0)<< ",  y " << bresenhams_line[i](1) <<std::endl;
        int index = map.get_index_from_xy(bresenhams_line[i](0), bresenhams_line[i](1));
            std::cout<<"indx" <<index<<",map value:  "<< map.data[index] <<std::endl;
        if(map.data[index] != 0 && map.data[index] !=-1){
            std::cout<<"indx" <<index<<", collision-- map value:  "<< map.data[index] <<std::endl;
            std::cout<<"collision!!- map-idx"<<index <<std::endl;
            return true;
        }
    }
    return false;
