...
public:
  ...
  bool transform_trjendpoint_to_globalframe(const geometry_msgs::Pose& inputPose, geometry_msgs::PoseStamped& pose_out)
  {
      geometry_msgs::PoseStamped pose_in;     //added by mk, ryan
      //pose_in.header.stamp=ros::Time(0);
      pose_in.header.stamp=ros::Time().now();
      //pose_in.header.stamp=ros::Time(0);
      pose_in.header.frame_id="base";
      pose_in.pose=inputPose;
      pose_in.pose.orientation.x=0.0;
      pose_in.pose.orientation.y=0.0;
      pose_in.pose.orientation.z=0.0;
      ...
      try{
        ...
        //listener.transformPose("map", ros::Time().now(), pose_in, pose_in.header.frame_id, pose_out);
        return true;
      }catch(tf::TransformException ex){
        std::cout << ex.what() << std::endl;
        sleep(0.1);
      ...
      //std::cout<<"-----checkpoint #0.75----"<<std::endl;
  ...
  void executeCB(const state_lattice_planner::LocalNaviGoalConstPtr &goal)
  {
    ...
    std::cout<<"Is this action active? " <<IsActive <<std::endl;
    ...
    std::vector<Eigen::Vector3d> local_trajectory;
    while(ros::ok() && IsActive && !as_.isPreemptRequested())
    {
        ...
        auto time_sec=ros::Time::now().toSec();
        ROS_INFO("trans_time: %.2lf", time_sec);
        //boostd::mutex
        //boost::mutex::scoped_lock lock(saved_mutex_);
        if(!bool_tf)
        {
            ROS_WARN("local goal transformation failed");
            bool_wait=true;
            failure_count++;
            ...
            else if(failure_count>3)
            {
                bool_asst_ctrl=true;
                //failure_count=0;
                //control with global pos without transform
            }
        }
        else
        {
            //control with transformed global pos
            bool_wait=false;
            failure_count=0;
        }
        if(!bool_wait)
        {
            ...
            if(pslp_planner->process(pose_base, is_finalgoal,global_pose, bool_replan, control_mode, local_trajectory))
            {
                callback_iter=0;
            ...
            else if(control_mode && (local_trajectory.size()>1))
            { 

                callback_iter++;
                std::cout<<"---------------------------------------"<<std::endl;
                std::cout<<"-----control with local_trajectory!----"<<std::endl;

                int mid_trj_size=local_trajectory.size();
                geometry_msgs::Pose trj_mid_pose;
                if(mid_trj_size>4)
                {
                    mid_trj_size=floor(local_trajectory.size()/2);
                    trj_mid_pose.position.x = local_trajectory[mid_trj_size][0];
                    trj_mid_pose.position.y = local_trajectory[mid_trj_size][1];
                }

                geometry_msgs::Pose trj_end_base;
                trj_end_base.position.x = local_trajectory.back()[0];
                trj_end_base.position.y = local_trajectory.back()[1];
                //bool bool_trans=transform_trjendpoint_to_globalframe(trj_end_base, pose_global);
                bool bool_trans=false;
                double robot_yaw=tf2::getYaw(m_pose.pose.orientation);
                double target_midangle_baselink= atan2(trj_mid_pose.position.y, trj_mid_pose.position.x);
                double target_angle_baselink= atan2(local_trajectory.back()[1],local_trajectory.back()[0]);
                //double control_target_yaw = robot_yaw+target_angle;
                //double target_angle =0.0;
                //std::cout<<"-----checkpoint #0----"<<std::endl;
                if(callback_iter%5==1)
                {
                    bool_trans=transform_trjendpoint_to_globalframe(trj_end_base, pose_global);
                    if(mid_trj_size>4)
                    {

                        if(target_angle_baselink>0)
                            target_angle = max(target_midangle_baselink, target_angle_baselink);
                        else
                            target_angle = min(target_midangle_baselink, target_angle_baselink);
                    }
                    else
                    {
                        target_angle=target_angle_baselink;
                    }
                ...
                if(!IsRotated){
                    //ROS_INFO("rotation");
                    //std::cout<<"robot_yaw: "<<robot_yaw <<", target_yaw: "<<target_angle <<std::endl;
                    if(CheckRotation(robot_yaw,target_angle,0.20))
                    {
                        IsRotated=true;
                        callback_iter=0;
                        ...
                        geometry_msgs::Twist vel_cmd;
                        if(fabs(target_angle)>0.32)
                            vel_cmd.angular.z=0.32*direction_z;
                        else
                            vel_cmd.angular.z=0.2*direction_z;

                        vel_cmd.linear.x=0.00; vel_cmd.linear.y=0.0;  vel_cmd.linear.z=0.0;
                        vel_cmd.angular.x=0.0; vel_cmd.angular.y=0.0; 
                        cmd_velocity_pub.publish(vel_cmd);
                    }
                }
                else{
                    ...
                    if(bool_trans)
                    {
                        ...
                        //std::cout<<"here-case1"<<std::endl;
                        std::vector<double> navtargetpose(2,0.0);
                        navtargetpose[0]=pose_global.pose.position.x;
                        navtargetpose[1]=pose_global.pose.position.y;
                        
                        if( checkDistance(global_pose,navtarget_pose,0.2))
                        {
                            ROS_INFO("local_goal succedded");
                            callback_iter=0;
                            IsRotated=false;
                            //return;
                        }
                        else
                        {
                            if(cango_forward)
                            {
                                ...
                                //std::cout<<"here-case2"<<std::endl;
                                geometry_msgs::Twist vel_cmd;
                                if(fabs(trj_end_base.position.y)>0.2)
                                {
                                    if(trj_end_base.position.y>0)
                                        vel_cmd.linear.y=0.125;
                                    else
                                        vel_cmd.linear.y=-0.125;
                                }
                                ...
                                if(trj_end_base.position.x<0.5)
                                    vel_cmd.linear.x=0.2;  
                                
                                vel_cmd.linear.z=0.0;
                                vel_cmd.angular.x=0.0; vel_cmd.angular.y=0.0; vel_cmd.angular.z=0.0;

                                //smoothing_velcommand(vel_cmd, previous_cmd);
                                cmd_velocity_pub.publish(vel_cmd);
                                previous_cmd=vel_cmd;
                            }
                            else{
                            
                                ROS_INFO("Cannot go Forward");
                                if(cango_backward)
                                {

                                ROS_INFO("Can go Backward!!");
                                if(cango_backward)
                                    vel_cmd.linear.x=-0.3;
                                    vel_cmd.linear.y=0.0;
                                    vel_cmd.linear.z=0.0;
                                    ...
                                    //find the alternative  plan

                                }
                                else
                                {

                                    ROS_INFO("Cannot go Backward");
                                    vel_cmd.linear.x=0.0;
                                    vel_cmd.linear.z=0.0;
                                    vel_cmd.angular.x=0.0; vel_cmd.angular.y=0.0;
                                    vel_cmd.angular.z=0.3*direction_z;
                                    cmd_velocity_pub.publish(vel_cmd);
                                    previous_cmd=vel_cmd;
                                    ROS_INFO("need_to_trun!!");
                                    //cango_forward=true;
                                    //cango_backward=false;
                                }
                            }
                            std::cout<<"here-case2-done"<<std::endl;
                        }
                    }
                    else
                    {

                            std::cout<<"here-case3-else"<<std::endl;
                            //Sending_zerovelcmd();
                            //vel_cmd.linear.x=0.12;  vel_cmd.linear.z=0.0;
                            //vel_cmd.angular.x=0.0; vel_cmd.angular.y=0.0; vel_cmd.angular.z=0.0;
                            //cmd_velocity_pub.publish(vel_cmd);
                            //previous_cmd=vel_cmd;
                        continue;
  ...
  void obstacle_check(const std::vector<double> global_pose)
  {

      if(!local_map_updated )
          return;

      bool isObstacle_ft, isObstacle_fl, isObstacle_fr, isObstacle_bk=false;

      geometry_msgs::Point front_point;
      front_point.x=0.475;
      front_point.y=0.0;
      geometry_msgs::Point fleft_point;
      fleft_point.x=0.45;
      fleft_point.y=0.1;
      geometry_msgs::Point fright_point;
      fright_point.x=0.45;
      fright_point.y=-0.1;
      ...
      std::vector<double> checkpoint_ft(2,0.0);
      std::vector<double> checkpoint_fl(2,0.0);
      std::vector<double> checkpoint_fr(2,0.0);
      std::vector<double> checkpoint_bk(2,0.0);

      checkpoint_ft[0] = global_pose[0]+front_point.x*cos(global_pose[2])-front_point.y*sin(global_pose[2]);
      checkpoint_ft[1] = global_pose[1]+front_point.x*sin(global_pose[2])+front_point.y*cos(global_pose[2]);
      int map_idx=Coord2CellNum(checkpoint_ft[0], checkpoint_ft[1],dynamic_map);

      if(dynamic_map.data[map_idx]!=0)
        isObstacle_ft=true;
      else
        isObstacle_ft=false;
    
      checkpoint_fl[0] = global_pose[0]+fleft_point.x*cos(global_pose[2])-fleft_point.y*sin(global_pose[2]);
      checkpoint_fl[1] = global_pose[1]+fleft_point.x*sin(global_pose[2])+fleft_point.y*cos(global_pose[2]);
      map_idx=Coord2CellNum(checkpoint_fl[0], checkpoint_fl[1],dynamic_map);
      ...
      //std::cout<<"fl: x: "<< checkpoint_fl[0] << ", y: " << checkpoint_fl[1] << ", occupied: "<< isObstacle_fl <<std::endl;
      checkpoint_fr[0] = global_pose[0]+fright_point.x*cos(global_pose[2])-fright_point.y*sin(global_pose[2]);
      checkpoint_fr[1] = global_pose[1]+fright_point.x*sin(global_pose[2])+fright_point.y*cos(global_pose[2]);
      map_idx=Coord2CellNum(checkpoint_fr[0], checkpoint_fr[1],dynamic_map);
      if(dynamic_map.data[map_idx]!=0)
        isObstacle_fr=true;
      else
        isObstacle_fr=false;

      //std::cout<<"fr: x: "<< checkpoint_fr[0] <<", y: "<<checkpoint_fr[1]<<", occupied: "<<isObstacle_fr <<std::endl;
      checkpoint_bk[0] = global_pose[0]+back_point.x*cos(global_pose[2])-back_point.y*sin(global_pose[2]);
      checkpoint_bk[1] = global_pose[1]+back_point.x*sin(global_pose[2])+back_point.y*cos(global_pose[2]);
      map_idx=Coord2CellNum(checkpoint_bk[0], checkpoint_bk[1],dynamic_map);
      if(dynamic_map.data[map_idx]!=0)
        isObstacle_bk=true;
      else
        isObstacle_bk=false;

      //std::cout<<"bk: x: "<< checkpoint_bk[0] <<", y: "<<checkpoint_bk[1]<<", occupied: "<<isObstacle_bk <<std::endl;
      cango_forward=true;
      if(( isObstacle_fr && isObstacle_fl))
          cango_forward=false;
      //if(( isObstacle_ft && isObstacle_fr) || (isObstacle_ft && isObstacle_fl) || isObstacle_ft || (isObstacle_fl && isObstacle_fr) )
          //cango_forward=false;
      //else
          //cango_forward=true;

      cango_backward=isObstacle_bk;
      return;
  ...
  void global_pose_callback(const geometry_msgs::PoseStamped::ConstPtr& msg)
  {
      ...
      //std::cout<<"global_pose_callback"<<std::endl;
      m_pose=*msg;
...
void global_map_callback(const nav_msgs::OccupancyGridConstPtr& msg)
{
    ...
    void Idx2Globalpose(int idx, std::vector<double>& global_coord, nav_msgs::OccupancyGrid map_)
    {
        global_coord.resize(2,0.0);

        int res = (int) (idx/ map_.info.width);
        int div = (int) (idx%map_.info.width);

        global_coord[0]=res*map_.info.resolution+map_.info.origin.position.x;
        global_coord[1]=div*map_.info.resolution+map_.info.origin.position.y;
    }
    ...
    int Coord2CellNum(double _x, double _y, nav_msgs::OccupancyGrid map_)
    {	
        ...
        reference_origin_x=map_.info.origin.position.x;
        reference_origin_y=map_.info.origin.position.y;
        ...
        dynamicCoord[0]=map_.info.origin.position.x+map_.info.resolution*target_Coord[0]+0.5*map_.info.resolution;
        dynamicCoord[1]=map_.info.origin.position.y+map_.info.resolution*target_Coord[1]+0.5*map_.info.resolution;

        int index= target_Coord[0]+map_.info.width*target_Coord[1];
...
protected:
  ...
  geometry_msgs::Twist previous_cmd;
