...
public:
    ...
    //std::vector<double>get_xy_from_index(int);
    void get_xy_from_index(int, std::vector<double>& );
...
void ObstacleMap<ELEMENT_TYPE>::set_shape(unsigned int width_, unsigned int height_, double resolution_)
{
    ...
    std::cout << "map origin x:"<< origin_x<<", origin y"<<origin_y<< std::endl;
}
...
template<typename ELEMENT_TYPE>
double ObstacleMap<ELEMENT_TYPE>::get_width(void) const
{
    return width;
}

template<typename ELEMENT_TYPE>
//void  ObstacleMap<ELEMENT_TYPE>::get_xy_from_index(int idx, double& x_, double& y_)
//std::vector<double> ObstacleMap<ELEMENT_TYPE>::get_xy_from_index(int idx_)
void ObstacleMap<ELEMENT_TYPE>::get_xy_from_index(int idx_, std::vector<double>& Coord)
{
    //std::vector<double> coord(2,0.0);
    Coord.resize(2,0.0);
    int res=(int)idx_/width;
    int div=(int)idx_%width;
    //x_=(div+0.5)*resolution;
    //y_=(res+0.5)*resolution;

    Coord[0]=(div+0.5)*resolution+origin_x;
    Coord[1]=(res+0.5)*resolution+origin_y;
    return;
}
